...
func TestSingleThreadSolverTSPDummy(t *testing.T) {
	size := 4
	...
	solver := gobnb.SingleThreadSolver{Problem: tsp}
...
func TestSingleThreadSolverTSPSimple(t *testing.T) {
	size := 4
	...
	bestState := &TravellingSalespersonProblemState{}
	solution.LoadState(bestState)
	assert.Equal(t, []int{0, 2, 1, 3}, bestState.Sequence, "should be 0/2/1/3 as best path")
}

func TestSingleThreadSolverTSP(t *testing.T) {
...
func TestMultiThreadSolverTSP(t *testing.T) {
	...
	solver := gobnb.MultiThreadSolver{Problem: tsp, NWorker: 3}
	_, objective, _, _ := solver.Solve(&gobnb.SolverConfigs{Mode: gobnb.DepthFirst, MaxIterCount: 2000000})
	assert.Equal(t, 291.0, objective, "should be 291 as best objective in ~40s")
}
